...
| `crates/infra-<provider>` (none yet) | One transport or provider adapter: admission, budgets, mapping to feature-owned types. | Business rules, config precedence, other adapters' policy. |
...
infra-http, infra-telemetry, health, service-config
...
Ownership choices made in stages 1 to 3 that a later change should not
silently reopen:
