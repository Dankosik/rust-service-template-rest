...
async fn inspect_query(Query(filters): Query<Filters>, TypedHeader(agent): TypedHeader<UserAgent>) -> Json<serde_json::Value> {
...
async fn typed_header_and_query_extractors_compose_on_axum() {
    ...
    let response = server.get("/tags?tag=rust&tag=backend")
...
fn url_query_encoding_does_not_use_string_concatenation() {
    ...
    target.query_pairs_mut().append_pair("q", "Rust & HTTP").append_pair("page", "2");
    assert_eq!(target.query_pairs().collect::<Vec<_>>(), [("q".into(), "Rust & HTTP".into()), ("page".into(), "2".into())]);
...
async fn a_reusable_http_client_has_explicit_timeouts_and_no_hidden_retry_stack() {
    ...
    let client = reqwest::Client::builder()
        ...
        .build().unwrap();
    let target = url::Url::parse(&mock.uri()).unwrap().join("items/1").unwrap();
    let body: serde_json::Value = client.get(target).send().await.unwrap()
