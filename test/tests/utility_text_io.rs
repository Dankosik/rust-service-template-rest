...
fn names_and_text_patterns_use_existing_utilities() {
    ...
    static JOB_ID: LazyLock<regex::Regex> = LazyLock::new(|| regex::Regex::new(r"^job-([0-9]+)$").unwrap());
...
fn byte_strings_do_not_require_lossy_utf8_conversion() {
    ...
    assert_eq!(input.lines().collect::<Vec<_>>(), [b"ok".as_slice(), b"invalid:\xff".as_slice()]);
...
fn recursive_file_operations_keep_errors_and_relative_paths() {
    ...
    let entries = walkdir::WalkDir::new(dir.path()).follow_links(false)
        .into_iter().collect::<Result<Vec<_>, _>>().unwrap();
    let files = entries.iter().filter(|entry| entry.file_type().is_file())
        .map(|entry| entry.path().strip_prefix(dir.path()).unwrap().to_owned())
        .sorted().collect_vec();
    assert_eq!(files, [std::path::PathBuf::from("nested/child.txt"), std::path::PathBuf::from("top.txt")]);
    assert_eq!(fs_err::read_to_string(dir.path().join("top.txt")).unwrap(), "top");
...
fn non_utf8_paths_are_rejected_when_converted_to_utf8_paths() {
    ...
    assert_eq!(camino::Utf8PathBuf::from_path_buf(path.clone()).unwrap_err(), path);
}
...
fn csv_preserves_commas_quotes_and_embedded_newlines() {
    let rows = [
        ImportRow { id: 1, label: "comma, and \"quote\"".into() },
        ImportRow { id: 2, label: "two\nlines".into() },
    ];
    ...
    let decoded = csv::Reader::from_reader(encoded.as_slice())
        .deserialize::<ImportRow>().collect::<Result<Vec<_>, _>>().unwrap();
