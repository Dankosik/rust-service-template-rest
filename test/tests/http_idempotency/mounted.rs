...
impl Provider {
    ...
    fn verifier(&self) -> Verifier {
        let transport = FixtureTransport::new(FIXTURE_HOST, self.address, &self.fixture_root)
    ...
    /// Stop serving and join every fixture server task.
    async fn stop(self) {
        self.cancel.cancel();
        self.tasks.close();
